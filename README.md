...
> 課題公開後は [docs/day1-checklist.md](docs/day1-checklist.md) を上から埋める（仕様読取り→テンプレ選択→
> 検証ループ配線→富岳初回 run→候補保存）。
...
│   ├── solver_naive.cpp    # 参照実装の骨組み (parse→solve_naive()→output。stress.py の naive)
...
│   ├── gen_small_cases.py  # seed固定の小ケース生成 (gen_case/corner_cases。stress.py と共有)
│   ├── validate_output.py  # 出力の形式/制約検査+score 雛形 (read_input/parse_output/validate/score)
│   ├── stress.py           # Fast vs Naive 比較 (--mode exact|valid-only|score-compare, --validator, --timeout)
│   ├── benchmark.py        # Fast の速度計測
│   ├── save_candidate.sh   # 提出候補を submissions/ へ保存 (秘密除外。命綱の複製)
...
├── submissions/            # .gitignore済み（save_candidate.sh の提出候補。.gitkeep のみ追跡）
├── build/                  # .gitignore済み
