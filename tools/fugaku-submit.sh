...
#   注: case の *[!class]* は改行も「class外の1文字」として捕捉するため、改行混入も自動で弾く。
...
_cfg_opt_path() {  # 任意・パス系 (空可): 英数 . _ / @ % - のみ (spath の %j 等を許容)
    local name="$1" val="${2-}"
    [ -n "$val" ] || return 0
    case "$val" in *[!A-Za-z0-9._/@%-]*)
...
# 任意 PJM ノブ (空なら未使用)。job script / PJM directive へ素で入るため改行・異常文字を弾く。
...
date -u +%Y-%m-%dT%H:%M:%SZ >> "\${RESULTS}/meta.txt"
# 一意入力は results/input.dat に複製済みなので remote inputs/ から掃除 (ゴミ蓄積防止)
if [ "${REMOTE_INPUT}" != "/dev/null" ]; then rm -f "${REMOTE_INPUT}" 2>/dev/null || true; fi
exit \${EXIT_CODE}
ENDJOB
)
